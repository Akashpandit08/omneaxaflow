...
@router.post("", response_model=ApiKeyCreateOut, status_code=status.HTTP_201_CREATED, dependencies=[RequireRole(["owner", "admin"])])
async def create_api_key(body: ApiKeyCreate, current_user: CurrentUser, workspace: CurrentWorkspace, db: DBSession):
...
@router.get("", response_model=List[ApiKeyOut], dependencies=[RequireRole(["owner", "admin"])])
...
@router.delete("/{api_key_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[RequireRole(["owner", "admin"])])
async def revoke_api_key(api_key_id: int, current_user: CurrentUser, workspace: CurrentWorkspace, db: DBSession):
    result = await db.execute(
